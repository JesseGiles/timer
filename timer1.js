// Implement an alarm clock / timer which will beep after a specified amount of time has passed. The user can specify an unlimited number of alarms using command line arguments

// node timer1.js 10 3 5 15 9

// This will make it beep at:

// 3 seconds
// 5 seconds
// 9 seconds
// 10 seconds
// 15 seconds

// A quick Google Search of "node system beep" shows that in order to make our machine perform a system sound, we can simply output the following special character to stdout.

// process.stdout.write('\x07');

const args = process.argv.slice(2);

const simpleTimer = () => {

  if (args.length === 0) {
    console.log("Please enter a number(s)"); //if no input, error
  }
 
  for (const inputs of args) {

    if (inputs < 0 || isNaN(inputs)) { //if negative # or NaN used, do nothing
      continue;
    } else {
      let timer = inputs; //loop arguments, set timer to input value on each pass
      setTimeout(() => {
        process.stdout.write('\u0007'); //beep sound
      }, timer * 1000); //make sound at time of # inputted * 1000ms
    }

  }

};

simpleTimer();


// Edge Cases
// With every software project, developers need to think about edge cases.

// Instruction
// Confirm that our script can handle some common edge cases.

// For this app, we can think of at least these three:

// No numbers are provided: Easy. It should just not beep at all and end immediately since no beeps should get scheduled.
// An input is a negative number: Ignore/skip any numbers that are negative. We can't schedule anything in the past.
// An input is not a number: Ignore/skip these as well, instead of attempting to call setTimeout with a non-number.